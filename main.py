...
def main():
    ...
    # AI functions for white and black
    ai_white = None  # Set to None to play as white
    ai_black = get_best_move_minimax  # Using minimax for black
    ...
    while not chess_board.is_game_over():
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and turn == 'w' and ai_white is None:
                ...
                if selected_piece is None:
                    if chess_board.piece_at(square) and chess_board.piece_at(square).color == chess.WHITE:
...
def display_winner(screen, winner, ai_white, ai_black):
    font = pygame.font.SysFont(None, 74)
    text = font.render(winner, True, GREEN)
    text_rect = text.get_rect(center=(WIDTH / 2, HEIGHT / 2))
    screen.blit(text, text_rect)

    ai_info = f"White: {ai_white.__name__}, Black: {ai_black.__name__}"
    font_info = pygame.font.SysFont(None, 36)
    text_info = font_info.render(ai_info, True, GREEN)
    text_info_rect = text_info.get_rect(center=(WIDTH / 2, HEIGHT / 2 + 50))
...
# Display AI names during the game
def display_ai_names(screen, ai_white, ai_black):
    ai_info = f"White: {ai_white.__name__}, Black: {ai_black.__name__}"
    font_info = pygame.font.SysFont(None, 36)
    text_info = font_info.render(ai_info, True, GREEN)
    text_info_rect = text_info.get_rect(center=(WIDTH // 2, 20))
    screen.blit(text_info, text_info_rect)
...
def main():
    ...
    draw_board(screen)
    draw_pieces(screen, board, images)
    display_ai_names(screen, ai_white, ai_black)
    pygame.display.flip()
    ...
    # Determine the outcome and display the winner
    if chess_board.is_checkmate():
        winner = "White Wins!" if chess_board.turn == chess.BLACK else "Black Wins!"
    ...
    display_winner(screen, winner, ai_white, ai_black)
...
# Constants
WIDTH, HEIGHT = 800, 900  # Increased height to add space for the display below the board
SQUARE_SIZE = WIDTH // 8
FPS = 60
...
def display_winner(screen, winner, ai_white, ai_black):
    ...
    screen.fill(WHITE, (0, 800, WIDTH, 100))  # Clear the area below the board
...
# Display AI names below the board during the game
def display_ai_names(screen, ai_white, ai_black):
    font = pygame.font.SysFont(None, 36)
    ai_info_white = f"White (GROK): {ai_white.__name__}"
    ai_info_black = f"Black (ChatGPT): {ai_black.__name__}"
...
def main():
    ...
    # Determine the outcome and display the winner
    if chess_board.is_checkmate():
        winner = "GROK Wins!" if chess_board.turn == chess.BLACK else "ChatGPT Wins!"
    elif chess_board.is_stalemate() or chess_board.is_insufficient_material() or chess_board.is_seventyfive_moves() or chess_board.is_variant_draw():
        winner = "Draw!"
    else:
        winner = "Game Over!"
